...
namespace {
    ...
    __global__ void GenerateRay(
        ...
        // Convert radar FoV to Radians
        const float vert_fov=DegToRad(height_fov);
        const float horiz_fov=DegToRad(width_fov);
        
        // Compute local base
        const float w_x = (rx_dir_x) / length(rx_dir_x, rx_dir_y, rx_dir_z);
        const float w_y = (rx_dir_y) / length(rx_dir_x, rx_dir_y, rx_dir_z);
        const float w_z = (rx_dir_z) / length(rx_dir_x, rx_dir_y, rx_dir_z);
        ...
        if (pixel_index < width * height) {
            ...
            const float horiz_angle = -horiz_fov + (horiz_fov*2) * (__int2float_rd(x) + 0.5)/ __int2float_rd(width);
            const float vert_angle = -vert_fov + (vert_fov*2) * (__int2float_rd(y) + 0.5)/ __int2float_rd(height);
            const float view_plane_x = tan(horiz_angle);
            const float view_plane_y = tan(vert_angle);
            ...
            pixel_distances[pixel_index] = (2*tan(horiz_fov)) / __int2float_rd(width); 
    ...
    __device__ float ValueAt(
        ...
        // Compute voxel size
        float voxel_size = (bounding_box_max_y - bounding_box_min_y) / (grid_res_y - 1);
        ...
        // Check if this is not nan inside grid
        if (ASSUME_TSDF) {
            if (grid[flat(min_index_x, min_index_y, min_index_z, grid_res_x, grid_res_y, grid_res_z)] < UNDEF_VAL+10 || 
            grid[flat(min_index_x, min_index_y+1, min_index_z, grid_res_x, grid_res_y, grid_res_z)] < UNDEF_VAL+10 ||
            grid[flat(min_index_x, min_index_y, min_index_z+1, grid_res_x, grid_res_y, grid_res_z)] < UNDEF_VAL+10 ||
            grid[flat(min_index_x, min_index_y+1, min_index_z+1, grid_res_x, grid_res_y, grid_res_z)] < UNDEF_VAL+10 ||
            grid[flat(min_index_x+1, min_index_y, min_index_z, grid_res_x, grid_res_y, grid_res_z)] < UNDEF_VAL+10 ||
            grid[flat(min_index_x+1, min_index_y+1, min_index_z, grid_res_x, grid_res_y, grid_res_z)] < UNDEF_VAL+10 ||
            grid[flat(min_index_x+1, min_index_y, min_index_z+1, grid_res_x, grid_res_y, grid_res_z)] < UNDEF_VAL+10 ||
            grid[flat(min_index_x+1, min_index_y+1, min_index_z+1, grid_res_x, grid_res_y, grid_res_z)] < UNDEF_VAL+10 
            ) {
                return MIN_STEP;
            }
    ...
    __global__ void Intersect(
        ...
        // Compute voxel size
        const float voxel_size = (bounding_box_max_y - bounding_box_min_y) / (grid_res_y - 1);
        ...
        if (pixel_index < width * height) {
            ...
            for (int steps = 0; steps < max_steps; steps++) {
               ...
               if (distance < voxel_size && distance) {
...
std::vector<at::Tensor> ray_matching_cuda(
                           const at::Tensor w_h_3, //Only used for size
                           const at::Tensor w_h, //Only used for size
    ...
    at::Tensor origins = torch::zeros_like(w_h_3);
    at::Tensor directions = torch::zeros_like(w_h_3);
    at::Tensor origin_image_distances = torch::zeros_like(w_h);
    at::Tensor pixel_distances = torch::zeros_like(w_h);
    ...
    at::Tensor voxel_position = torch::zeros_like(w_h_3);
    at::Tensor intersection_pos = torch::zeros_like(w_h_3);
