...
__global__ void cuda_image_kernel(float* device_p_xyz_r, 
    ...
    int original_ind = ind; // Back up for safety if needed, though we calc x,y,z from ind
    
    // ind maps to z + y*NZ + x*NY*NZ in original code?
    // Let's re-verify indexing logic from original file:
    // int z = ind % NUM_Z; ind /= NUM_Z; int y = ind % NUM_Y; ind /= NUM_Y; int x = ind % NUM_X;
    // Flattened index is x*NY*NZ + y*NZ + z
    
    int z = ind % NUM_Z;
    ind /= NUM_Z;
    int y = ind % NUM_Y;
    ind /= NUM_Y;
    int x = ind; // remaining is x

    if (original_ind >= NUM_X*NUM_Y*NUM_Z || x < 0 || y < 0 || z < 0 || x >= NUM_X || y >= NUM_Y || z >= NUM_Z) { return; }
    ...
    for (uint i = 0; i < NUM_ANTENNAS; i++) {
        ...
        if (use_interpolated_processing != 0){ // Apply faster but approximate coorelation
            ...
            float real_phase = cosf(-2 * pi * distance / wavelength); // Use cosf/sinf
            ...
            for (uint j = 0; j < SAMPLES_PER_MEAS; j++) {
                // Correlate this sample with the complex value of the expected measurement
                float real_meas = device_measurements_r[i*SAMPLES_PER_MEAS+j];
                float imag_meas = device_measurements_i[i*SAMPLES_PER_MEAS+j];
                float real_phase = cosf(-2 * pi * distance / wavelength + -2 * pi * distance * j * slope / SPEED_LIGHT);
                float imag_phase = sinf(-2 * pi * distance / wavelength + -2 * pi * distance * j * slope / SPEED_LIGHT);
                sum_r += mult_r(real_meas, imag_meas, real_phase, imag_phase);
                sum_i += mult_i(real_meas, imag_meas, real_phase, imag_phase);
            }
        }
...
void cuda_image_launcher(
    ...
    int total_voxels = num_x * num_y * num_z;
    int blocks = (total_voxels + threads_per_block - 1) / threads_per_block;
    
    // If start_ind is used for multi-gpu offset, we might need to adjust logic, but 
    // PyTorch parallelism usually handles devices differently. 
    // For now we assume this call processes the whole assigned chunk.
    // However, the kernel uses start_ind to offset the index. 
    
    cuda_image_kernel<<<blocks, threads_per_block>>>(
        ...
        wavelength,
        fft_spacing,
        num_x, num_y, num_z, num_ant, num_rx_ant, samples_per_meas,
        start_ind, is_ti_radar, use_interpolated_processing
    );
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("cuda_image", &cuda_image_launcher, "Imaging GPU Kernel");
}